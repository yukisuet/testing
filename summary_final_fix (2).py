def analyze_account_balances(excel_file_path, output_file_path=None):
    ...
    2. Filter to keep only transactions divisible by 29.95
    ...
    # Filter for transactions divisible by 29.95
    print("Filtering for transactions divisible by 29.95...")
    before_filter = len(combined_data)
    
    # Use modulo to check if divisible by 29.95 (remainder should be very close to 0)
    divisible_mask = abs(combined_data['TRANS_AM'] % 29.95) < 1e-10
    ...
    if divisible_mask.any():
        ...
        print(f"Sample amounts being kept (divisible by 29.95): {sample_kept}")
        
        # Show the multiples (how many times 29.95 goes into each amount)
        sample_multiples = [round(amt / 29.95) for amt in sample_kept]
        print(f"Corresponding multiples of 29.95: {sample_multiples}")
    
    if (~divisible_mask).any():
        sample_filtered = combined_data[~divisible_mask]['TRANS_AM'].head(5).tolist()
        print(f"Sample amounts being filtered out: {sample_filtered}")
    
    combined_data = combined_data[divisible_mask]
    after_filter = len(combined_data)
    print(f"Kept {after_filter} rows out of {before_filter} (removed {before_filter - after_filter} rows not divisible by 29.95)")
    
    if after_filter == 0:
        print("WARNING: No transactions are divisible by 29.95! Check your data.")
        return None
