def analyze_account_balances(excel_file_path, output_file_path=None):
    ...
    # Check data types and memory usage for large datasets
    print(f"TRANS_AM data type: {combined_data['TRANS_AM'].dtype}")
    print(f"Memory usage: {combined_data.memory_usage(deep=True).sum() / 1024**2:.2f} MB")
    
    # Check for any problematic values
    null_count = combined_data['TRANS_AM'].isnull().sum()
    inf_count = np.isinf(combined_data['TRANS_AM']).sum()
    zero_count = (combined_data['TRANS_AM'] == 0).sum()
    
    print(f"Null values in TRANS_AM: {null_count}")
    print(f"Infinite values in TRANS_AM: {inf_count}")
    print(f"Zero values in TRANS_AM: {zero_count}")
    
    if null_count > 0 or inf_count > 0:
        print("Removing null/infinite values...")
        combined_data = combined_data[~(combined_data['TRANS_AM'].isnull() | np.isinf(combined_data['TRANS_AM']))]
        print(f"Rows after removing null/inf: {len(combined_data)}")
    
    # Calculate quotient and decimal part for each transaction amount
    print("Calculating quotients and decimal parts...")
    
    try:
        # Use explicit float64 for precision with large numbers
        quotients = combined_data['TRANS_AM'].astype('float64') / 29.95
        decimal_parts = quotients - np.floor(quotients)
        
        print(f"Quotients calculated successfully. Shape: {quotients.shape}")
        print(f"Decimal parts calculated successfully. Shape: {decimal_parts.shape}")
    ...
    print(f"Sample calculations (first {sample_size} rows):")
    for i in range(sample_size):
        amount = combined_data.iloc[i]['TRANS_AM']
        quotient = quotients.iloc[i]
        decimal_part = decimal_parts.iloc[i]
        print(f"  Row {i}: Amount: {amount}, Quotient: {quotient:.6f}, Decimal: {decimal_part:.6f}")
    
    # Filter OUT numbers where decimal part is close to 0.999 (almost multiples)
    threshold = 0.99
    print(f"Applying threshold filter: decimal_parts > {threshold}")
    
    try:
        almost_multiple_mask = decimal_parts > threshold
        print(f"Almost multiple mask created successfully. Shape: {almost_multiple_mask.shape}")
        print(f"Type of mask: {type(almost_multiple_mask)}")
        
        # Keep numbers that are NOT almost multiples
        divisible_mask = ~almost_multiple_mask
        print(f"Divisible mask created successfully. Shape: {divisible_mask.shape}")
        
    except Exception as e:
        print(f"Error creating masks: {e}")
        return None
    
    # Count results BEFORE applying filter
    almost_count = almost_multiple_mask.sum()
    keep_count = divisible_mask.sum()
    
    print(f"Threshold for filtering: {threshold}")
    print(f"Rows that will be filtered out (decimal > {threshold}): {almost_count}")
    print(f"Rows that will be kept (decimal <= {threshold}): {keep_count}")
    print(f"Total accounted for: {almost_count + keep_count} (should equal {len(combined_data)})")
    
    # Show sample of amounts being kept vs filtered out (if any exist)
    if keep_count > 0:
        try:
            kept_indices = divisible_mask[divisible_mask].index[:5]
            sample_kept = combined_data.loc[kept_indices, 'TRANS_AM'].tolist()
            sample_kept_quotients = [round(q, 6) for q in (combined_data.loc[kept_indices, 'TRANS_AM'] / 29.95)]
            print(f"Sample amounts being kept: {sample_kept}")
            print(f"Their quotients (÷29.95): {sample_kept_quotients}")
        except Exception as e:
            print(f"Error showing kept samples: {e}")
    
    if almost_count > 0:
        try:
            filtered_indices = almost_multiple_mask[almost_multiple_mask].index[:5]
            sample_filtered = combined_data.loc[filtered_indices, 'TRANS_AM'].tolist()
            sample_filtered_quotients = [round(q, 6) for q in (combined_data.loc[filtered_indices, 'TRANS_AM'] / 29.95)]
            ...
            print(f"Error showing filtered samples: {e}")
    
    # Apply the filter with explicit error handling
    print("Applying filter to dataset...")
    try:
        combined_data_filtered = combined_data[divisible_mask].copy()
        after_filter = len(combined_data_filtered)
        ...
        print(f"Mask shape: {divisible_mask.shape}")
        print(f"Data shape: {combined_data.shape}")
        return None
    
    print(f"FINAL RESULT: Kept {after_filter} rows out of {before_filter} (removed {before_filter - after_filter} rows)")
    
    if after_filter == before_filter:
        print("WARNING: No rows were filtered! This suggests the filter didn't work.")
        print("Checking some decimal parts manually...")
        for i in range(min(5, len(decimal_parts))):
            dp = decimal_parts.iloc[i]
            print(f"  Decimal part {i}: {dp:.6f}, > {threshold}? {dp > threshold}")
