def analyze_account_balances(excel_file_path, output_file_path=None):
    ...
    # Filter OUT rows where the quotient is "almost" a whole number (decimal part > 0.99)
    # This filters out numbers like 137680.14 (quotient = 4596.999666)
    # but keeps numbers like 100.00 (quotient = 3.338898)
    threshold = 0.99
    almost_whole_mask = combined_data['temp_decimal_part'] > threshold
    
    print(f"\nFiltering logic:")
    print(f"- Filter OUT: Quotients with decimal part > {threshold} (almost whole numbers)")
    print(f"- KEEP: Quotients with decimal part <= {threshold} (clearly not whole or exactly whole)")
    
    # Count what will be filtered
    rows_to_filter = almost_whole_mask.sum()
    rows_to_keep = (~almost_whole_mask).sum()
    
    print(f"\nFilter results:")
    print(f"Rows to filter out (decimal > {threshold}): {rows_to_filter}")
    print(f"Rows to keep (decimal <= {threshold}): {rows_to_keep}")
    
    # Show samples before filtering
    if rows_to_filter > 0:
        filtered_sample = combined_data[almost_whole_mask][['TRANS_AM', 'temp_quotient', 'temp_decimal_part']].head()
        print(f"\nSample rows being FILTERED OUT:")
    ...
    if rows_to_keep > 0:
        kept_sample = combined_data[~almost_whole_mask][['TRANS_AM', 'temp_quotient', 'temp_decimal_part']].head()
        print(f"\nSample rows being KEPT:")
